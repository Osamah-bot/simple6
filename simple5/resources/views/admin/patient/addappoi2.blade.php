    ...
    <section id="appointment" class="appointment section-bg">
        <div class="container">
            ...
            <div class="profile-img">
                <a href="/patientprofile" class="avatar"><img alt="ERROR" src="{{ asset('storage/patient_images/'.$result->img_url) }}"></a>
                <?php //	src="'.asset("storage/purchases/".$purchase->image).'"
                ?>
            </div>
            <form action="{{ route('admin.appoistore') }}" method="POST" role="form" class="php-email-form">
                @csrf
                <h4>Patient Info</h4>
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 form-group mt-3">
                        <select name="doctor_id" id="doctor" class="form-select">
                            ...
                            @if (count($doctorsname))
                                @foreach ($doctorsname as $docname)
                                    <option value="{{ $docname->doctor_id }}"> {{ $docname->fname . ' ' . $docname->lname }}
                                    </option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div class="col-md-4 form-group mt-3">
                        <select name="Room" id="" class="form-select">
                            ...
                            @if (count($room))
                                @foreach ($room as $r)
                                    <option value="{{ $r->room_id }}">{{ $r->roomtype . ' ' . $r->roomno }}</option>
                                @endforeach
                            @endif
                        </select>
                        <div class="validate"></div>
